    ...
    def LLMmove(self):
        ...
        if self.current_winner is None:
            ...
            if int(response['row']) * 3 + int(response['column']) in available_moves:
                ...
                self.validprompt = f'''Your previous response was '{{"row": {int(response['row'])}, “column": {int(response['column'])}}}'. This move was deemed invalid for the following reason: 'Already Taken'. Please adjust accordingly. Here are the instructions again:'''
                self.LLMmove()
